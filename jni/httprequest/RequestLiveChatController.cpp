...
RequestLiveChatController::RequestLiveChatController(HttpRequestManager *phttprequestManager, RequestLiveChatControllerCallback callback/* CallbackManager* pCallbackManager*/) {
...
void RequestLiveChatController::onSuccess(long requestId, string url, const char* buf, int size) {
	...
	if( url.compare(CHECK_COUPON_PATH) == 0 ) {
		...
		if( mRequestLiveChatControllerCallback.onCheckCoupon != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onUseCoupon != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onQueryChatVirtualGift != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onQueryChatRecord != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onQueryChatRecordMutiple != NULL ) {
		...
		if (bFlag) {
			...
			TiXmlNode *infoNode = rootNode->FirstChild(LIVECHAT_PHOTOINFO);
			bFlag = item.Parsing(infoNode);
		}
		if( mRequestLiveChatControllerCallback.onSendPhoto != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onPhotoFee != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onGetPhoto != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onUploadVoice != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onPalyVoice != NULL ) {
			mRequestLiveChatControllerCallback.onPalyVoice(requestId, bFlag, errnum, errmsg, filePath);
		}
		...
		if( mRequestLiveChatControllerCallback.onSendGift != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onQueryRecentVideoList != NULL ) {
		...
		if( mRequestLiveChatControllerCallback.onGetVideoPhoto != NULL ) {
		...
		, const string& filePath)
{
	...
	// sizeType
	string strSizeType("");
	if (0 <= sizeType && sizeType < _countof(GETPHOTO_PHOTOSIZE_PROTOCOL)) {
