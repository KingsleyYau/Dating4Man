...
long RequestProfileController::UpdateProfile(int weight, int height, int language, int ethnicity,
	...
	if( weight > -1 ) {
		if( weight != 0 ) {
			sprintf(temp, "%d", weight + PROFILE_WEIGHT_BEGINVALUE);
		} else {
	...
	if( height > -1 ) {
		if( height != 0 ) {
			sprintf(temp, "%d", height + PROFILE_HEIGHT_BEGINVALUE);
		} else {
...
long RequestProfileController::SaveContact(string telephone, int telephone_cc, string landline,
	...
	if( telephone_cc > -1 ) {
		entiy.AddContent(PROFILE_TELEPHONE_CC, CountryArray[telephone_cc]);
	}
	...
	if( landline_cc > -1 ) {
		entiy.AddContent(PROFILE_LANDLINE_CC, CountryArray[landline_cc]);
	}
