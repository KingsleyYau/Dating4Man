...
#ifndef PROFILEITEM_H_
#define PROFILEITEM_H_
	...
	void Parse(Json::Value root) {
		if( root.isObject() ) {
			...
			if( root[PROFILE_HEIGHT].isString() ) {
				height = atoi(root[PROFILE_HEIGHT].asString().c_str()) - PROFILE_HEIGHT_BEGINVALUE;
			}

			if( root[PROFILE_WEIGHT].isString() ) {
				weight = atoi(root[PROFILE_WEIGHT].asString().c_str()) - PROFILE_WEIGHT_BEGINVALUE;
			}
			...
			if( root[PROFILE_ETHNICITY].isString() ) {
				ethnicity = atoi(root[PROFILE_ETHNICITY].asString().c_str());
			}
			...
			if( root[PROFILE_MOBILE_CC].isString() ) {
				mobile_cc = atoi(root[PROFILE_MOBILE_CC].asString().c_str());
			}
			...
			if( root[PROFILE_LANDLINE_CC].isString() ) {
				landline_cc = atoi(root[PROFILE_LANDLINE_CC].asString().c_str());
			}
