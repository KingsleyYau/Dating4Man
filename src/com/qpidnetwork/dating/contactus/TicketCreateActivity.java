...
public class TicketCreateActivity extends BaseActionBarFragmentActivity
		implements MaterialTextField.OnFocuseChangedCallback,
		MaterialDialogSingleChoice.OnClickCallback, OnRequestCallback {
	...
	protected void onCreate(Bundle arg0) {
		...
		contactFormOptions = this.getResources().getStringArray(
				R.array.contact_ticket_form);
		initTitle();
		initViews();
	}

	private void initTitle() {
		getCustomActionBar().setTitle(getString(R.string.ticket_create_new),
				getResources().getColor(R.color.text_color_dark));
		getCustomActionBar().setAppbarBackgroundColor(
				getResources().getColor(R.color.theme_actionbar_secoundary));
		getCustomActionBar().setTouchFeedback(
				MaterialAppBar.TOUCH_FEEDBACK_HOLO_LIGHT);
		getCustomActionBar().addButtonToLeft(R.id.common_button_back, "back",
				R.drawable.ic_close_grey600_24dp);
		getCustomActionBar().addButtonToRight(R.id.common_button_save, "save",
				R.drawable.ic_done_grey600_24dp);
	}
	...
	private void initViews() {
		...
		tvProblem.setText(getString(R.string.contact_form)
				+ contactFormOptions[0]);
		tvProblem.getEditor().setSingleLine();
		tvProblem.getEditor().setEllipsize(TruncateAt.END);

		tvProblem.setOnFocusChangedCallback(this);

		contactFormSelectDialog = new MaterialDialogSingleChoice(this,
				contactFormOptions, this);
		contactFormSelectDialog
				.setTitle(getString(R.string.What_is_your_problem_relatedto));
	...
	private void onNewTicketCreated() {
		...
		RequestOperator.newInstance(this).AddTicket(typeId, subject, body,
				filePath, this);
	}
	...
	protected void handleUiMessage(Message msg) {
		...
		case CREATE_NEW_TICKET_SUCCESS:
			...
			FlatToast.showStickToast(TicketCreateActivity.this, "Done!",
					FlatToast.StikyToastType.DONE);
			finish();
			break;
		case CREATE_NEW_TICKET_FAILED:
			RequestFailBean errorBean = (RequestFailBean) msg.obj;
			FlatToast.showStickToast(TicketCreateActivity.this, "Failed!",
					FlatToast.StikyToastType.FAILED);
			Toast.makeText(TicketCreateActivity.this, errorBean.errmsg,
		...
		if (resultCode == RESULT_OK) {
			...
			case RESULT_LOAD_IMAGE_ALBUMN:
				...
				if (null != data) {
					...
					filePath = CompatUtil.getSelectedPhotoPath(this,
							selectedImage);
					if (!StringUtil.isEmpty(filePath)
							&& new File(filePath).exists()) {
						Bitmap temp = ImageUtil.decodeAndScaleBitmapFromFile(
								filePath, UnitConversion.dip2px(this, 100),
								UnitConversion.dip2px(this, 120));
	...
	public void onClick(AdapterView<?> adptView, View v, int which) {
		String contactFormString = getString(R.string.contact_form)
				+ contactFormOptions[which];
	...
	public void onFocuseChanged(View v, boolean hasFocus) {
		if (hasFocus) {
			if (contactFormSelectDialog != null
					&& !contactFormSelectDialog.isShowing())
				contactFormSelectDialog.show();
			etSubject.getEditor().requestFocus();
		}
	}
