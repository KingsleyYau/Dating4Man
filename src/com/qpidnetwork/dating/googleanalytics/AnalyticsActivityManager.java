...
//		ReportScreenPath(activity);
	...
	public String ReportScreenPath(Activity activity)
	{
		...
		// 设置旧Screen路径
		if (!StringUtil.isEmpty(screenPath)) {
	 ...
	 * @param activity			activity
	 * @param keyWordsSupport	是否支持关键字替换（是：替换ScreenName中的关键字，否：仅去掉关键字）
	...
	private String GetScreenName(Activity activity, boolean keyWordSupport, String filterTagName)
	{
		...
		{	
			...
			if (screenName.isEmpty()
				&& !StringUtil.isEmpty(preActivityScreenName)) 
			{
				...
				&& !tagNameList.isEmpty())
			{
				...
				{
					// 过滤tagName
					if (!StringUtil.isEmpty(filterTagName)
			...
			if (!screenName.isEmpty()) 
			{
					...
					&& screenName.contains(keyWord))
				{
					...
					if (keyWordSupport)
					{
						...
						if (!StringUtil.isEmpty(parentScreenName)) {
