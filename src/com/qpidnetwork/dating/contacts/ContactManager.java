...
public class ContactManager implements OnLoginManagerCallback,
		...
		mHandler = new Handler() {
			public void handleMessage(android.os.Message msg) {
				...
				case REQUEST_LIVECHAT_NOTIFICATION_SUCCESS: {
					...
					LiveChatNotification nt = LiveChatNotification
				...
				case REQUEST_LIVECHAT_KICK_OFF: {
					...
					QpidApplication.kickOffType = KickOfflineType.values()[msg.arg1];
				...
				case REQUEST_LIVECHAT_EMF_CONATCT_UPDATE: {
					...
					LadyDetailManager.getInstance().QueryLadyDetail(usrId,
					...
					new OnLadyRecentContactListCallback() {
						...
						public void OnLadyRecentContactList(boolean isSuccess,
							...
							if (isSuccess) {
								...
								Log.d("contact",
										"ContactManager::getContacts() synchronized mContactList begin");
								synchronized (mContactList) {
									mContactList.clear();
									mContactsMap.clear();
									if (listArray != null) {
								...
								Log.d("contact",
										"ContactManager::getContacts() synchronized mContactList end");
								/* 联系人下载成功开始更新在线状态 */
	...
	private List<ContactBean> getFavoritesList() {
		List<ContactBean> tempList = new ArrayList<ContactBean>();
		Log.d("contact",
				"ContactManager::getFavoritesList() synchronized mContactList begin");
		synchronized (mContactList) {
			if (mContactList != null) {
				for (ContactBean bean : mContactList) {
					if (bean.isfavorite) {
		...
		Log.d("contact",
				"ContactManager::getFavoritesList() synchronized mContactList end");
		return tempList;
	}
	...
	public void updateFavoriteStatus(String womanId, boolean isFavorite) {
		Log.d("contact",
				"ContactManager::updateFavoriteStatus() synchronized mContactList begin");
		synchronized (mContactList) {
			if (mContactsMap.containsKey(womanId)) {
		...
		Log.d("contact",
				"ContactManager::updateFavoriteStatus() synchronized mContactList end");
	}
	...
	public void addOrUpdateContact(LoveCallBean lovecallItem) {
		...
		Log.d("contact",
				"ContactManager::addOrUpdateContact() synchronized mContactList begin");
		synchronized (mContactList) {
		...
		Log.d("contact",
				"ContactManager::addOrUpdateContact() synchronized mContactList end");
		onContactListUpdateCallback();
	 ...
	 * @param lady
	 * @param localFlag
	 *            是否添加到本地存储（男士邀请联系人需要，防止刷新后聊天对象找不到）
	 */
	public void updateBySendEMF(LadyDetail lady) {
		ContactBean bean = null;
		Log.d("contact",
				"ContactManager::updateBySendEMF() synchronized mContactList begin");
		synchronized (mContactList) {
		...
		Log.d("contact",
				"ContactManager::updateBySendEMF() synchronized mContactList end");
		onContactListUpdateCallback();
	}
	 ...
	 *            是否男士主动邀请，需保存到本地联系人
	 */
	public void updateByMessage(final LCUserItem userItem,
			final boolean isInvite) {

		/* 在聊或者试聊时，添加到联系人列表,没有女士头像及firstname，需获取后添加 */
		LadyDetailManager.getInstance().QueryLadyDetail(userItem.userId,
				new OnLadyDetailManagerQueryLadyDetailCallback() {
							...
							synchronized (mContactList) {
								...
								if (!mContactsMap.containsKey(bean.womanid)) {
									...
									if (isInvite) {
										...
										if (mLocalContactMap
												.containsKey(localKey)) {
											mLocalContactMap.get(localKey).add(
													bean);
										} else {
											List<ContactBean> localList = new ArrayList<ContactBean>();
											localList.add(bean);
											mLocalContactMap.put(localKey,
													localList);
										}
									}
								}
	 ...
	 *            处理由男士端发起的邀请，添加到联系人
	 */
	private void updateOrAddContact(final LCUserItem userItem,
			final boolean isInvite) {
		ContactBean bean = null;
		if (userItem != null) {
			Log.d("contact",
					"ContactManager::updateOrAddContact() synchronized mContactList begin");
			synchronized (mContactList) {
				if (mContactsMap.containsKey(userItem.userId)) {
					...
					if (StringUtil.isEmpty(bean.msgHint)) {
						...
						|| isInvite) {
					Message msg = Message.obtain();
					msg.what = RequestFlag.REQUEST_LIVECHAT_MESSAGE_CONATCT_UPDATE
							.ordinal();
			...
			Log.d("contact",
					"ContactManager::updateOrAddContact() synchronized mContactList end");
		}
	}
		...
		Runnable task = new Runnable() {
			...
			public void run() {
				...
				if (isNeedUpdateContactList(item)) {
					...
					Log.d("contact",
							"ContactManager::onReceiveMessage() synchronized mContactList begin");
					synchronized (mContactList) {
						if (mWomanId != null) {
					...
					Log.d("contact",
							"ContactManager::onReceiveMessage() synchronized mContactList end");
					onContactListUpdateCallback();
				}
	...
	private boolean isNeedUpdateContactList(LCMessageItem item) {
		...
		Log.d("contact",
				"ContactManager::isNeedUpdateContactList() synchronized mContactList begin");
		synchronized (mContactList) {
			if (!mContactsMap.containsKey(item.fromId)
		...
		Log.d("contact",
				"ContactManager::isNeedUpdateContactList() synchronized mContactList end");
		return isUpdate;
	}
