...
public class AdAnakysisManager {
	...
	public AdAnakysisManager(Context context){
		...
		if(mAnalysisItem == null){
			mAnalysisItem = new AnalysisItem();
		}
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				for(OnRequestCallback callback : mUtmCallbackList){
					callback.OnRequest(response.isSuccess, response.errno, response.errmsg);
				}
	...
	private void saveUtmReferenceData(Context context, AnalysisItem item){
		...
		try {
	        ...
	        String utmReferenceBase64 = new String(Base64.encode(baos.toByteArray(), Base64.DEFAULT));  
	        
	        FileOutputStream outStream = context.openFileOutput(getUtmReferencePath(), Context.MODE_PRIVATE);
	...
	private AnalysisItem getUtmReferenceData(){
	    ...
	    try {  
	        ...
	        byte[] base64Bytes = Base64.decode(utmReferenceBase64.getBytes(), Base64.DEFAULT);  
	...
	public void summitUtmReference(OnRequestCallback requestCallback){
		...
		RequestJniOther.InstallLogs(RequestJni.GetDeviceId(tm), dm.widthPixels, dm.heightPixels, installTime, submitTime, QpidApplication.versionCode, reference, new OnRequestCallback() {
