 ...
 * @author Max.Chiu
 */
public class EMFAttachmentPreviewActivity extends BaseActivity implements OnPageChangeListener {
	 ...
	 * 分页适配器
	 */
	private class EMFAttachmentActivityPagerAdapter extends FragmentStatePagerAdapter {
		// 当前显示的
		...
		public Fragment getItem(final int arg0) {
			...
			case NORAML_PICTURE: {
				...
				// 显示
				String localPath = "";
				if( item.photoLocalUrl != null && item.photoLocalUrl.length() > 0 ) {
					localPath = item.photoLocalUrl;
				} else {
					localPath = FileCacheManager.getInstance().CacheImagePathFromUrl(item.photoUrl);
				}
				//((EMFAttachmentPhotoFragment) fragment).ReloadData(item.photoUrl, localPath);
				
				fragment = new EMFAttachmentPhotoFragment(arg0, item.photoUrl, localPath);
			...
			case PRIVATE_PHOTO:{
				...
				// 创建新的
				fragment = new EMFAttachmentPrivatePhotoFragment(arg0, privatePhotoDirection);
				mEMFAttachmentPrivatePhotoragment.add(fragment);
				mIndexFragment.add(fragment);
				
				// 设置购买事件
				((EMFAttachmentPrivatePhotoFragment) fragment).SetOnClickBuyListener(new OnClickBuy() {
					@Override
					public void onClickBuy(EMFAttachmentPrivatePhotoFragment fragment) {
						...
						if( item.privatePhoto.photoFee ) {
							...
							if( file.exists() && file.isFile() ) {
								...
								// 请求接口获取私密照预览图
								PrivatePhotoView(item.privatePhoto, arg0, PrivatePhotoType.LARGE);
							}
						} else {
							// 请求付费
							InboxPhotoFee(item.privatePhoto, arg0, PrivatePhotoType.LARGE);
						}
					}
					...
					public void onClickDownload(
						...
						if( item.privatePhoto.photoFee ) {
							...
							// 生成缓存路径
							String localPhotoPathOriginal = FileCacheManager.getInstance().CachePrivatePhotoImagePath(
									item.privatePhoto.sendId, 
									item.privatePhoto.photoId,
							...
							if( file.exists() && file.isFile() ) {
								// 直接保存到相册
								String fileName = item.privatePhoto.womanid + "-" + System.currentTimeMillis() + ".jpg";
								SaveImageToGallery(localPhotoPath, localPhotoPathOriginal, fileName);
...
//								}
								...
								// 请求接口获取私密照原图
								PrivatePhotoView(item.privatePhoto, arg0, PrivatePhotoType.ORIGINAL);
							}
						} else {
							// 请求付费
							InboxPhotoFee(item.privatePhoto, arg0, PrivatePhotoType.ORIGINAL);
						}
					}
				});
				...
				if( item.privatePhoto.photoFee ) {
					...
					// 没有购买, 清空显示
					((EMFAttachmentPrivatePhotoFragment) fragment).ReloadData("", "", 1);
				}
			}break;
			case VIRTUAL_GIFT:{
				// 虚拟礼物
				...
				mIndexFragment.add(fragment);
				((EMFAttachmentVirtualGiftFragment) fragment).SetInsert(mbShowInsert);
				((EMFAttachmentVirtualGiftFragment) fragment).SetTips(mbShowTips);
				
				// 生成虚拟礼物的视频图片路径
				String photoUrl = VirtualGiftManager.getInstance().GetVirtualGiftImage(item.vgId);
				String localPhotoPath = VirtualGiftManager.getInstance().CacheVirtualGiftImagePath(item.vgId);
				String videoUrl = VirtualGiftManager.getInstance().GetVirtualGiftVideo(item.vgId);
				String localVideoPath = VirtualGiftManager.getInstance().CacheVirtualGiftVideoPath(item.vgId);
				
				// 虚拟礼物描述
				String format = getResources().getString(R.string.emf_virtual_gift_name_credit);
				String name = VirtualGiftManager.getInstance().GetVirtualGiftName(item.vgId);
	...
	public static Intent getIntent(Context context, ArrayList<EMFAttachmentBean> attachment, int currIndex){
	...
	protected void onCreate(Bundle arg0) {
		...
		if(bundle != null){
			if(bundle.containsKey(EMF_ATTACHMENT_LIST)){
				mAttachList = bundle.getParcelableArrayList(EMF_ATTACHMENT_LIST);
			}
			...
			if (bundle.containsKey(ATTACH_DIRECTION)){
				privatePhotoDirection = PrivatePhotoDirection.valueOf(bundle.getString(ATTACH_DIRECTION));
			}
		}
		
		if (privatePhotoDirection == null) privatePhotoDirection = PrivatePhotoDirection.WM;
	...
	public void InitView() {
        ...
        // 分页控件
		mAdapter = new EMFAttachmentActivityPagerAdapter(getSupportFragmentManager());
...
//			if( item.getIndex() == index || item.getIndex() < (index - 1) || item.getIndex() > (index + 1) ) {
	 ...
	 * 请求购买私密照片
	 */
	public void InboxPhotoFee(final PrivatePhotoBean item, final int index, final PrivatePhotoType type) {
		// 此处应有菊花
		showProgressDialog("Loading...");
		RequestOperator.getInstance().InboxPhotoFee(
				item.womanid, 
				item.photoId,
				item.sendId, 
				item.messageid, 
				new OnEMFInboxPhotoFeeCallback() {
					...
					public void OnEMFInboxPhotoFee(boolean isSuccess, String errno,
						...
						if( isSuccess ) {
							// 成功
							msg.what = RequestFlag.REQUEST_BUY_SUCCESS.ordinal();
							msg.arg1 = index;
	 ...
	 * 获取已购买的私密照
	 */
	public void PrivatePhotoView(PrivatePhotoBean item, final int index, final PrivatePhotoType type) {
		// 此处应有菊花
		showProgressDialog("Loading...");
		...
		long requestId = RequestOperator.getInstance().PrivatePhotoView(
				...
				new OnEMFPrivatePhotoViewCallback() {
					...
					public void OnEMFPrivatePhotoView(boolean isSuccess, String errno,
						...
						if( isSuccess ) {
							// 成功
							msg.what = RequestFlag.REQUEST_GET_PHOTO_SUCCESS.ordinal();
							obj.type = type;
							msg.arg1 = index;
						} else {
							// 失败
							msg.what = RequestFlag.REQUEST_GET_PHOTO_FAIL.ordinal();;
						}
						msg.obj = obj;
						mHandler.sendMessage(msg);
					}
		...
		mHandler = new Handler() {
			...
			public void handleMessage(android.os.Message msg) {
				...
				case REQUEST_BUY_SUCCESS:{
					...
					PrivatePhotoView(callbackItem.privatePhotoBean, msg.arg1, callbackItem.type);
					if(callbackItem.type == PrivatePhotoType.ORIGINAL) {
						PrivatePhotoView(callbackItem.privatePhotoBean, msg.arg1, PrivatePhotoType.LARGE);
					}
				}break;
				case REQUEST_BUY_FAIL:{
					...
					default: {
						Toast.makeText(mContext, callbackItem.errmsg, Toast.LENGTH_LONG).show();	
						break;
					}
					}
				...
				case REQUEST_GET_PHOTO_SUCCESS:{
					...
					if( callbackItem.type == PrivatePhotoType.LARGE ) {
						// 获取大图, 刷新界面
						EMFAttachmentPrivatePhotoFragment fragment = (EMFAttachmentPrivatePhotoFragment) GetFragmentInList(mAdapter.mEMFAttachmentPrivatePhotoragment, msg.arg1);
						...
						// 获取原图, 下载到相册
						localPhotoPathOriginal = FileCacheManager.getInstance().CachePrivatePhotoImagePath(
								item.privatePhoto.sendId, 
								item.privatePhoto.photoId,
								PrivatePhotoType.ORIGINAL
								);
...
//		showProgressDialog("Loading...");
	    ...
	    try {
	    	...
	    	Cursor actualimagecursor = this.mContext.managedQuery(uri,proj,null,null,null);  
	    ...
	    Toast.makeText(this, getString(R.string.livechat_saved_origional_image), Toast.LENGTH_LONG).show();
