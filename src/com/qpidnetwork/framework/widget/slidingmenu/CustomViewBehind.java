...
import com.qpidnetwork.framework.widget.slidingmenu.SlidingMenu.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
	...
	private static final int MARGIN_THRESHOLD = 48; // dips  //针对模式SlidingMenu.TOUCHMODE_MARGIN设置的有效触摸区域
	...
	/**
	 * 在SlidingMenu.TOUCHMODE_MARGIN模式下，判断是否点击有效区域
	 * @param content
	 * @param x
	 * @return
	 */
	...
	public boolean menuOpenTouchAllowed(View content, int currPage, float x) {
		Log.i("test slidingmenu", "menuOpenTouchAllowed the currPage is�� " + currPage);
		switch (mTouchMode) {
		case SlidingMenu.TOUCHMODE_FULLSCREEN:
			Log.i("test slidingmenu", "menuOpenTouchAllowed TOUCHMODE_FULLSCREEN ");
			return true;
	...
	public boolean menuTouchInQuickReturn(View content, int currPage, float x) {
		Log.i("test slidingmenu", "menuTouchInQuickReturn the currPage is�� " + currPage);
	...
	/**
	 * 判断菜单未打开状态下，手指滑动是否菜单滑动
	 * @param dx 手指滑动距离
	 * @return
	 */
	public boolean menuClosedSlideAllowed(float dx) {
		Log.i("test slidingmenu", "menuClosedSlideAllowed the x is�� " + dx);
		if (mMode == SlidingMenu.LEFT) {
			return dx > 0;
		} else if (mMode == SlidingMenu.RIGHT) {
			return dx < 0;
	...
	/**
	 * 判断菜单已打开状态下，手指滑动是否菜单滑动
	 * @param dx 手指滑动距离
	 * @return
	 */
	public boolean menuOpenSlideAllowed(float dx) {
		Log.i("test slidingmenu", "menuOpenSlideAllowed the x is�� " + dx);
		if (mMode == SlidingMenu.LEFT) {
			return dx < 0;
		} else if (mMode == SlidingMenu.RIGHT) {
			return dx > 0;
	...
	public void drawSelector(View content, Canvas canvas, float openPercent) {
		...
		if (mSelectorDrawable != null && mSelectedView != null) {
			String tag = (String) mSelectedView.getTag(R.id.widget_slidingmenu_selected_view);
	...
	public void setSelectedView(View v) {
		if (mSelectedView != null) {
			mSelectedView.setTag(R.id.widget_slidingmenu_selected_view, null);
			mSelectedView = null;
		}
		if (v != null && v.getParent() != null) {
			mSelectedView = v;
			mSelectedView.setTag(R.id.widget_slidingmenu_selected_view, TAG+"SelectedView");
			invalidate();
		}
	}
