...
import com.qpidnetwork.framework.widget.slidingmenu.SlidingMenu.OnClosedListener;
import com.qpidnetwork.framework.widget.slidingmenu.SlidingMenu.OnOpenedListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	private Scroller mScroller;//获取当前滑动位置，配合实现自定义view的滑动操作
	 ...
	 */
	protected VelocityTracker mVelocityTracker;//flinging和gestures手势事件滑动频率问题
	 ...
	 * 设置当前选中页
	 * @param item  当前页（item）
	 * @param smoothScroll 是否滑滚到（有无滑动过程）
	 * @param always  是否一直重绘制
	 * @param velocity  速度
	 */
	...
	public boolean isMenuOpen() {
		Log.i("test slidingmenu", "isMenuOpen mCurItem is :" + mCurItem);
	...
	//结束滚动，如果正在滚动，停止滚动相关操作，直接跳到目标x,y
	...
	private boolean thisSlideAllowed(float dx) {
		...
		Log.i("test slidingmenu", "thisSlideAllowed the x is�� " + dx);
		return allowed;
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			break;
		case MotionEvent.ACTION_MOVE:
			Log.i("test slidingmenu", "onTouchEvent MotionEvent.ACTION_MOVE");
			if (!mIsBeingDragged) {	
				determineDrag(ev);
				if (mIsUnableToDrag)
					return false;
