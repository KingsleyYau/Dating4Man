...
public class RangeSeekBar<T extends Number> extends ImageView {
	...
	private final float lineHeight = 3 * getResources().getDisplayMetrics().density;
	...
	public static final int BACKGROUND_COLOR = Color.parseColor("#abb3ba");
	...
	public RangeSeekBar(T absoluteMinValue, T absoluteMaxValue, Context context)
		...
		drawThumbNormalImage();
		drawThumbPressedImage();
	...
	private void drawThumbNormalImage(){
		//create thumb image
				float density = this.getContext().getResources().getDisplayMetrics().density;
				
				
				int thm_size = (int)(36.0f * density);
				int thm_normal_size = (int)(24.0f * density);
				int thm_point_size = (int)(12.0f * density);
				
				thumbImage = Bitmap.createBitmap(thm_size, thm_size, Config.ARGB_8888);
				
				Paint paint = new Paint();
				paint.setAntiAlias(true);
				
				Canvas canvas = new Canvas();
				canvas.setBitmap(thumbImage);
				
				//create point
				paint.setColor(getResources().getColor(R.color.green));
				canvas.drawCircle(thm_size / 2, thm_size / 2, thm_point_size / 2, paint);
				
				paint.setColor(getResources().getColor(R.color.transparent_green));
				canvas.drawCircle(thm_size / 2, thm_size / 2, thm_normal_size / 2, paint);
	}
	
	private void drawThumbPressedImage(){
		//create thumb image
				float density = this.getContext().getResources().getDisplayMetrics().density;
				...
				thumbPressedImage = Bitmap.createBitmap(thm_size, thm_size, Config.ARGB_8888);
				
				Paint paint = new Paint();
				paint.setAntiAlias(true);
				
				Canvas canvas = new Canvas();
				canvas.setBitmap(thumbPressedImage);
				...
				paint.setColor(getResources().getColor(R.color.transparent_green));
				canvas.drawCircle(thm_size / 2, thm_size / 2, thm_size / 2, paint);
	}
	...
	protected synchronized void onDraw(Canvas canvas) {
		...
		if (!IS_MULTI_COLORED) {
			...
			canvas.drawRoundRect(rect, lineHeight / 2.0f, lineHeight / 2.0f, paint);
