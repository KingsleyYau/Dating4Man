...
public class LCInviteManager {
	...
	private void RemoveOverTimeInvite()
	{
		...
		while (i < mInviteUserList.size()) 
		{
			...
			if (userItem.msgList.size() > 0) {
				LCMessageItem item = userItem.msgList.get(0);
				int currentTime = (int)(System.currentTimeMillis() / 1000);
				if (item.createTime + mHandleTimeInterval >= currentTime) {
					removeFlag = false;
				}
			}
			else {
	 ...
	 * @param message
	 */
	public synchronized LCMessageItem HandleInviteMessage(
			...
			, String message)
	{
				...
				|| mPreHandleTime + mHandleTimeInterval <= System.currentTimeMillis()))
		{
			...
			if (null != inviteUserItem
				&& !inviteUserItem.msgList.isEmpty()) 
			{
				// 添加到UserManager
